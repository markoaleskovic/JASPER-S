        ...
        public static List<OperationInfoViewModel> GetOperationsInfo(Type container)
        {
            ...
            foreach (var method in container.GetMethods())
            {
                if (method.IsStatic)
                {
                    ...
                    if (para.Length == 2)
                    {
                        var delType = typeof(Func<bool, bool, bool>);
                        var del = (Func<bool, bool, bool>)Delegate.CreateDelegate(delType, method);
                    ...
                    else if (para.Length == 1)
                    {
                        if (para[0].ParameterType.IsArray)
                        {
                            ...
                            var delType = typeof(Func<bool[], bool>);
                            var del = (Func<bool[], bool>)Delegate.CreateDelegate(delType, method);
                    ...
                    {
                        var delType = typeof(Func<bool>);
                        var del = (Func<bool>)Delegate.CreateDelegate(delType, method);
